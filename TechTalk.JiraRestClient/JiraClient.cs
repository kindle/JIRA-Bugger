...
namespace TechTalk.JiraRestClient
{
    ...
    public class JiraClient<TIssueFields> : IJiraClient<TIssueFields> where TIssueFields : IssueFields, new()
    {
        ...
        {
            return EnumerateIssues(projectKey, null, null).ToArray();
        }

        public IEnumerable<Issue<TIssueFields>> GetIssues(String projectKey, String filter, String issueType)
        {
            return EnumerateIssues(projectKey, filter, issueType).ToArray();
        }
        ...
        public IEnumerable<Issue<TIssueFields>> EnumerateIssues(String projectKey, String filter, String issueType)
        {
            try
            {
                return EnumerateIssuesInternal(projectKey, filter, issueType);
            }
            catch (Exception ex)
            {
        ...
        private IEnumerable<Issue<TIssueFields>> EnumerateIssuesInternal(String projectKey, String filter, String issueType)
        {
            var queryCount = 50;
            var resultCount = 0;
            while (true)
            {
                ...
                if (!String.IsNullOrEmpty(issueType))
                    ...
                    if (filter.ToLowerInvariant().Equals("all"))
                    {
                        jql += String.Format("+AND+(reporter=currentUser()+OR+assignee=currentUser())"); 
                    }   
                    else if (filter.ToLowerInvariant().Equals("reporter"))
                    {
                        jql += String.Format("+AND+reporter=currentUser()"); 
                    }
                    else
                    {
                        jql += String.Format("+AND+assignee=currentUser()");
                    }
